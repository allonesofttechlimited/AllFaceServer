...
//	public static Thread _camera_thread;
//	public static Thread _drawing_thread;
...
//	//public Liveness2DEstimator.LivenessAndScore lmlm = new Liveness2DEstimator.LivenessAndScore();

//	//public Liveness2DEstimator.Liveness liveness_2d_estimator = new Liveness2DEstimator.Liveness();
//	//public Liveness2DEstimator _liveness_2d_estimator;
//	public Options options1;
//	public Liveness2DEstimator.LivenessAndScore liveness_2d_res_with_score;
//	public Liveness2DEstimator liveness_2d_estimator;
...
//	//public FacerecService facerecService;
//	//public FacerecService.Config vw_config;
...
//	//Liveness2DEstimator.LivenessAndScore = new Liveness2DEstimator.LivenessAndScore();
...
//		float frame_fps_limit, Liveness2DEstimator _liveness_2d_estimator, string val1, FacerecService service, Capturer capture, VideoCapture cam)
//	{
...
//		_database = database;
//		liveness_2d_estimator = _liveness_2d_estimator;
//		val = val1;
//		svc = service;
//		capturer = capture;
//		camera = cam;
...
//		//Liveness2DEstimator liveness2DEstimator = new Liveness2DEstimator();
...
//	//  userdata is supposed to be pointer to this Worker
//	private static void MatchFoundCallback(
...
//			Liveness2DEstimator.LivenessAndScore liveness_2d_result = worker.liveness_2d_estimator.estimate(sample);
//			//Console.WriteLine("liveness_2d_result #### : " + liveness_2d_result);

//			string score_str = Math.Round(liveness_2d_result.score, 3).ToString();

//			Result = liveness_2d_result.liveness == Liveness2DEstimator.Liveness.REAL ? score_str + " - real" :
...
//			Console.WriteLine("The Score is : " + Result);
//		}
...
//				int k = MyConstants.cnt;

//				if (element_id == (ulong)VideoWorker.MATCH_NOT_FOUND_ID)
//				{
//					MAssert.Check(i == 0);
//					Console.WriteLine("  {0}: MATCH NOT FOUND", i);
//					MyConstants.MyFirstConstant[k] = "MATCH NOT FOUND";
//					MyConstants.MyFirstConstant[MyConstants.cnt] = "&&&&&&&&&&&&&&" + i + " ^^^^^^^";
//					//val = "MATCH NOT FOUND";
//				}
//				else
//				{
//					MAssert.Check(element_id < (UInt64)worker._database.names.Count);
//					string j = worker._database.names[(int)element_id] + "&&" + Result;

//					MyConstants.MyFirstConstant[k] = j;
//					MyConstants.matchedwith[k] = worker._database.names[(int)element_id];

//					string[] checkscore = new string[3];
//					checkscore = Result.Split(' ');
//					string score = checkscore[0];

//					double currentscore = double.Parse(score);

//					MyConstants.livescore[k] = currentscore;
...
//					if (k == 14)
//					{
//						VideoRecognitionDemo.matchedWith(worker._database.names[(int)element_id]);
//						worker.Dispose();
//						capturer.Dispose();
//						svc.Dispose();
//						camera.Dispose();
//						//_drawing_thread.Abort();
//						//_camera_thread.Abort();
...
//			else if (MyConstants.cnt >= 15)
//			{
//				//System.;
//				//MyConstants.cnt = 0;
//				//Array.Clear(MyConstants.livescore, 0, MyConstants.livescore.Length);
//				//Array.Clear(MyConstants.matchedwith, 0, MyConstants.matchedwith.Length);
//				//Array.Clear(MyConstants.MyFirstConstant, 0, MyConstants.MyFirstConstant.Length);
...
//		//
//		//Object obj = new Liveness2DEstimator.LivenessAndScore();
...
//					//byte[] data =frame.image.GetRawData();
//					//Marshal.Copy(frame.image.Ptr, data, 0, (int)data.Length);
...
//			OpenCvSharp.Scalar.All(0));
//		//Emgu.CV.Mat res = new Emgu.CV.Mat(Math.Max(data.frame.Rows+data.depth.Rows,stripe_height));
...
//			unsafe
//			{
//				UInt16* src_ptr = (UInt16*)data.depth.Ptr;
...
//				for (int k = 0; k < (data.depth.IsEmpty ? 1 : 2); ++k)
//				{
